...
function in5_getPathFromUrl($directUrl)
{
  $uploadsUrl = in5_getIndexFileUrl();
  $uploadsDir = in5_getIn5UploadsFolder();
  $filePath   = substr($directUrl, strlen($uploadsUrl), strlen($directUrl));
  $withIndex  = $uploadsDir . $filePath;
  $final      = str_replace('index.html', '', $withIndex);
...
function in5_delete_file()
{
  ...
  $root_pos = strpos($archiveLocation,"in5-archives/");
  $root_piece = substr($archiveLocation, $root_pos+13);
  $mid_pos = strpos($root_piece,"/");
  $mid_piece = substr($root_piece, 0, $mid_pos);
  $last_piece = substr($archiveLocation, 0, $root_pos+13);
  $final_piece = $last_piece.$mid_piece."/";
  in5_rrmdir($final_piece);
